...
export class QuestionaryService {
  ...
  async saveQuestionary(
    request: CreateQuestionaryRequestDTO,
    token: string
  ): Promise<Questionary> {
    const email = this.loginService.getEmailOfToken(token);
    logger.info(`Save questionary for user ${email}`);
    const userPromise: Promise<Teacher> = this.teacherService.findByEmail(
      email
    );
    const questionaryPromise: Promise<
      Questionary
    > = this.createQuestionaryOfRequest(request);
    return Promise.all([userPromise, questionaryPromise])
      .then((value) => {
  ...
  async createQuestionaryOfRequest(
    request: CreateQuestionaryRequestDTO
  ): Promise<Questionary> {
    ...
    const questions: Question[] = request.questions.map(
      (question: QuestionTO) => {
  ...
  findQuestionaryWithTeacher(hash: string): Promise<Questionary> {
    ...
    return this.questionaryDao.findOne(
      { hash: hash },
      { relations: ["teacher"] }
    );
  }
  ...
  async findQuestionariesByEmail(email: string): Promise<Questionary[]> {
    ...
    if (isAdmin) {
      ...
      const teacher = await this.teacherService.findByEmailWithQuestionaries(
        email
      );
      if (!teacher) {
        ...
        logger.info(
          `Questionaries found ${
            teacher.createdQuestionaries != undefined
              ? teacher.createdQuestionaries.length
              : 0
          }`
        );
  ...
  async getOptionsOfQuestionary(
    ...
    const questionsPromise: Promise<
      Question[]
    > = this.questionService.getQuestionsOfQuestionaryWithOptions(hash);
  ...
  async deleteQuestionary(token: string, hash: string): Promise<ShallResult> {
    ...
    const email = this.loginService.getEmailOfToken(token);
    const questionary: Questionary = await this.findQuestionaryWithTeacher(
      hash
    );
    if (questionary) {
      ...
      await this.questionaryDao.delete({
        hash: hash,
      });
      return Promise.resolve(new ShallResult(true, "ok"));
    }
    return Promise.reject(new FailedResult("Cannot find the questionary"));
  }
  ...
  plusResponsesQuantityOfQuestionary(hash: string): Promise<UpdateResult> {
    return this.questionaryDao.increment(
      { hash: hash },
      "quantity_respones",
      1
    );
  ...
  showResultOfQuestionary(hash: string): Promise<UpdateResult> {
    return this.questionaryDao.update(
      { hash: hash },
